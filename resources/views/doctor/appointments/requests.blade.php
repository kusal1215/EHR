            ...
            <div class="col-md-12">
                <div class="table-responsive">
                    <table class="mydatadable table table-stripped" id="user_table">
                        ...
                        <tbody>
                            @foreach(Auth::user()->doctor_req_appointments as $key => $request_appointment)
                            <tr>
                                <td>{{ $key+1 }}</td>
                                <td><img width="28" height="28" src="/assets/img/user.jpg" class="rounded-circle m-r-5"
                                        alt="">{{ $request_appointment->patient->name }}</td>
                                <td>{{ $request_appointment->date }}</td>
                                <td>{{ $request_appointment->time }}</td>
                                <td class="text-right">
                                    <div class="dropdown dropdown-action">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item"
                                                href="{{route('DoctorAppointmentManager.appointments.add',$request_appointment->id)}}">
                                                <i class="fa fa-eye m-r-5"></i> Add Appointment
                                            </a>
                                        </div>
                                    </div>
                                </td>
